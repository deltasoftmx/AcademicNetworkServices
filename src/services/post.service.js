 ...
 * - group_id: number
 */
async function getBasePostDataUserAuth(post_id, user_id) {
  const query = `
    select
      posts.id,
      users.username,
 ...
 * - group_id: string
 */
async function getBasePostData(post_id) {
  const query = `
    select
      posts.id,
      users.username,
 ...
 */
async function getPostsForTimeline(user_id, offset = 10, page = 0) {
  const query = `
    select
      id,
      username,
 ...
 */
async function getPostDataUserAuth(post_id, user_id) {
  const query = `
    select
      posts.id,
      users.username,
 ...
 */
async function postBelongsToPrivateGroup(post_id) {
  const query = `
    select 
      user_groups.id as group_id,
 ...
 * @returns {boolean}
 */
async function userBelongsToGroup(user_id, group_id) {
  const query = `
    select id
